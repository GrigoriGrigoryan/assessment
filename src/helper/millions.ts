...
export default function millions (input: number, word: string, arr: string[]) {
    ...
    if ( arr.length === 7 ) return cardinals.one[arr[0]] + ' ' + cardinals.oneIndex[2] + ' ' + `${thousands(+arr.slice(1, 7).join(''), word, arr.slice(1, 7))}`;

    if ( arr. length === 8) return `${tens(+arr.slice(0, 2).join(''), word, arr.slice(0, 2))}` + ' ' + cardinals.oneIndex[2] + ' ' + thousands(+arr.slice(2, 8).join(''), word, arr.slice(2, 8));

    if ( arr.length === 9) return `${hundreds(+arr.slice(0, 3).join(''), word, arr.slice(0, 3))}` + ' ' + cardinals.oneIndex[2] + ' ' + thousands(+arr.slice(3, 9).join(''), word, arr.slice(3, 9));
