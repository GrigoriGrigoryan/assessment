...
export default function hundreds (input: number, word: string, arr: string[]) {
    ...
    if ( arr.reduce((a, b) => String(Number(a) + Number(b))) === arr[0] ) return cardinals.one[arr[0]] + ' ' + cardinals.twd[10];

    if ( input > 100 && input < 1000) return  cardinals.one[arr[0]] + ' ' + cardinals.twd[10] + ' and ' + `${lastWord(word, arr)}`;
}